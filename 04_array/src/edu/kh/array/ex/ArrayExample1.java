...
public class ArrayExample1 {
	...
	public void ex4() {
		...
		for(int i=0; i<score.length; i++) {
			...
			score[i] = sc.nextInt(); // score[i] = 100 80 ...


			sum += score[i]; // 배열에 저장된 값을 누적
			// sum = sum + 100, 80...
		}		
	...
	public void ex5() {
		...
		// char[] arr이 참조하는 배열 요소 A, B, C, D, E 대입하기
	...
	public void ex8() {
		...
		System.out.print("과일 입력 : "); // input 변수 = 오렌지 등...
		String input = sc.next();

		boolean flag = false; // 참, 거짓 flag 형성

		for(int i=0; i<arr.length; i++) { // 0~딸기~블루베리
			if(input.equals(arr[i])) { // input 과일변수와 arr변수가 같으면
				// String 비교시 equals() 사용!!
				System.out.println(i + "번째 인덱스에 존재");
				flag = true; // 일치하는 값이 존재하면 true로 변경
			} 
		}
	...
	public void ex9() {
		...
		// 2. 문자 하나를 입력 받아 일치하는 문자가 char 배열에 몇 개 존재하는 지 확인
		...
		System.out.print("문자열 입력 : "); // input 변수 = Hello
		...
		for(int i=0; i<arr.length; i++) {
			...
			//System.out.print(arr[i]+" "); 배열 잘 들어갔나 확인
		}
		...
		for(int i=0; i<arr.length; i++) { // 0~문자열
			System.out.print(arr[i]+" ");
